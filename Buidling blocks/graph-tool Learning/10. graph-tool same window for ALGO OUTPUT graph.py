...
class GraphWindowL(GraphWindow):
    ...
    def set_Position_and_Dimension(self, position_x=0, position_y=0, width=1 , height=1):
        ...
        Arguments:
            position_x {int} -- x cordinate of system's screen, where the top-left corner of the graph windows is to be placed
            position_y {int} -- y cordinate of system's screen, where the top-left corner of the graph windows is to be placed
...
class Log_Window(Gtk.Window):
    ...
    def on_button_start_algo_clicked(self, widget):
        ...
        print("Hello")
        Log_Window.t1 = threading.Thread(target=g.show_BFS, args=(
            graph_window, logger_window, Log_Window.algorithm_start_vertex,))
...
class GraphL(Graph):
    ...
    def render_wait_log(self, graph_window, logger_window, log_text='', seconds=1):
        ...
        logger_window.log_buffer.insert(
            logger_window.log_buffer.get_end_iter(), "> " + log_text + "\n")
        # print log_text
        # time.sleep(seconds)
    ...
    def show_BFS(self, graph_window, logger_window, root_vertex=0):
        ...
        self.set_filters(e_Is_in_BFT, v_Is_in_BFT)

        # Tree layout --root at center--- https://graph-tool.skewed.de/static/doc/draw.html#graph_tool.draw.radial_tree_layout
        tree_layout = radial_tree_layout(self, root_vertex)

        # graph_window.graph = GraphWidget(self, tree_layout)
        # graph_window.set_default_size(int(1366*.5), 600)        
        # graph_window.graph.fit_to_window(ink=False)
        # del graph_window
        # graph_window.destroy()
        graph_window = GraphWindowL(self, tree_layout,
        ...
        graph_window.set_Position_and_Dimension(0,0,.5,1) 
        ...
        # graph_window.graph.reset_layout()
        # graph_window.graph.layout_callback()
        # graph_window.graph.draw()
        # graph_window.graph.queue_resize()
        # graph_window.graph.pos_to_device(pos= tree_layout)
        # graph_window.graph.fit_to_window(ink=False)
        # sfdp_layout(self, pos=tree_layout, K=.5, init_step=1, max_iter=100)


        # rect = graph_window.graph.get_allocation()
        # print rect
        

        # graph_window.graph.size_allocate(Gdk.Rectangle(768, 1366
        #                     *.5, 0,0))
